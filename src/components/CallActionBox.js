...
const CallActionBox = ({ onHangupPress }) => {
  ...
  return (
    <View style={styles.buttonsContainer}>
      <Pressable onPress={() => console.warn('Reverse Camera')} style={styles.iconButton}>
        <Ionicons name="ios-camera-reverse" size={30} color="white" />
      </Pressable>

      <Pressable onPress={() => setIsCameraOn(prev => !prev)} style={styles.iconButton}>
        <MaterialIcons name={isCameraOn ? 'camera-off' : 'camera'} size={30} color="white" />
      </Pressable>

      <Pressable onPress={() => setIsMicOn(prev => !prev)} style={styles.iconButton}>
