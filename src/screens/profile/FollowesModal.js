...
export default function FollowersModal({ visible, onClose }) {
  ...
  // 🔹 Memoize filtered users to prevent infinite loop
  const filteredUsers = useMemo(() => {
    if (!allUsers) return [];
    return allUsers.filter((u) => {
      if (activeTab === "Followers") return !u.isFollowing;
      if (activeTab === "Following") return u.isFollowing;
      if (activeTab === "Suggested") return u.username !== currentUsername && !u.isFollowing;
      return true;
    });
  }, [allUsers, activeTab, currentUsername]);
